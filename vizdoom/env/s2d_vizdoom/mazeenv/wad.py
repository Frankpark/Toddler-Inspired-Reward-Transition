...
def generate_wads(prefix, wad, behavior, BLOCK_SIZE=96, script=None):
    ...
    for map_index, file_name in enumerate(
        ...
        with open(file_name) as map_source:
            map = [line.strip() for line in map_source.readlines()]
            map = [line for line in map if line]
        ...
        things, vertexes, linedefs = build_wall(map, BLOCK_SIZE)
