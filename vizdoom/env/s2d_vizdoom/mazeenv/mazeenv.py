...
class MazeEnv(VizDoom):
    def __init__(self, unique_maps=False, number_maps=1, size=(10, 10), random_spawn=False,
                 random_key_positions=False, seed=None, clip=(-1, 1), random_wall_texture=False, random_ceiling_texture=False, random_floor_texture=False,
                 floor_texture_list=["CEIL5_2"], ceiling_texture_list=["CEIL5_1"], wall_texture_list=["STONE2"],
                 ...
                 resolution="RES_160X120", non_target_break=False, target_break=True,
                 acs_path=None, keys_used_list=None,
        ...
        self.random_wall_texture = random_wall_texture
        self.random_floor_texture = random_floor_texture
        self.random_ceiling_texture = random_ceiling_texture
        self.wall_texture_list = wall_texture_list
        self.floor_texture_list = floor_texture_list
        self.ceiling_texture_list = ceiling_texture_list
        ...
        super().__init__(self.cfg_path, number_maps=self.number_maps, scaled_resolution=self.scaled_resolution,
                         ...
                         gen_map=self.gen_map, helper_rwd_config=None)
    ...
    def generate_mazes(self):
        ...
        write_acs(random_player_spawn=self.random_spawn,
                  ...
                  random_wall_texture=self.random_wall_texture,
                  random_floor_texture=self.random_floor_texture,
                  random_ceiling_texture=self.random_ceiling_texture,
                  wall_texture_list=self.wall_texture_list,
                  floor_texture_list=self.floor_texture_list,
                  ceiling_texture_list=self.ceiling_texture_list,
                  seed=self.seed,
                  ...
                  min_spawn_dist_from_player=self.min_spawn_dist_from_player)
    ...
    @staticmethod
    def load_vizdoom_env(mazes_path, number_maps, num_obj_to_spawn, helper_rwd_config, min_spawn_dist_from_player=0.0, action_frame_repeat=4, scaled_resolution=(42, 42),
                         living_reward=-0.0025, target_reward=5.0, non_target_penalty=1.0,
                         timeout_penalty=1.0, non_target_break=False, target_break=True):
        """
        ...
        if str(mazes_path).endswith(".cfg"):
            return VizDoom(mazes_path, number_maps=number_maps, 
                           helper_rwd_config=helper_rwd_config,
                           ...
                           num_obj_to_spawn=num_obj_to_spawn, target_break=target_break,
                           min_spawn_dist_from_player=min_spawn_dist_from_player)
        else:
            cfg_paths = list(Path(mazes_path).glob("*.cfg"))
            if len(cfg_paths) != 1:
                raise ValueError("Invalid number of cfgs within the mazes path: ", len(cfg_paths))
            return VizDoom(cfg_paths[0], number_maps=number_maps, 
                           helper_rwd_config=helper_rwd_config,
                           scaled_resolution=scaled_resolution,
                           action_frame_repeat=action_frame_repeat, living_reward=living_reward,
                           target_reward=target_reward, non_target_penalty=non_target_penalty,
